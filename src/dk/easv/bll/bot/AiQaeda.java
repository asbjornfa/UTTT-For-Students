...
public class AiQaeda implements IBot {
    ...
    private IMove calculateMove(IGameState state, int maxTimeMs) {
        ...
        while (System.currentTimeMillis() < time + maxTimeMs) {
            ...
            // Should start by putting the first piece in the middle of the macroboard and the middle of the microboard.
            for (IMove move : legalMoves) {
                if (move.getX() == 4 && move.getY() == 4) {
                    return move; // Place the first piece in the middle of the macroboard and the microboard
                }
            }

            // If no strategic moves are found, make a random move
